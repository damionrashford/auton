...
from fast_mcp_agent.bridge import MCPBridge, PlaywrightProcess
from fast_mcp_agent.config import Settings, get_settings
...
from fast_mcp_agent.mcp.middleware import attach_middleware
from fast_mcp_agent.models import AgentStatus
from fast_mcp_agent.storage import NeonConversationStore, create_pool, run_migrations
...
async def agent_lifespan(server: FastMCP):
    ...
    if pool is not None:
        ...
        from fast_mcp_agent.core.conversation import ConversationStore as _InMemStore
    ...
    if pool is not None:
        try:
            from fast_mcp_agent.storage.memory import MemoryStore

            memory_store = MemoryStore(pool=pool, llm=llm)

            from fast_mcp_agent.storage.memory_tools import MEMORY_TOOLS, handle_memory_tool
    ...
    if settings.blockchain_enabled and settings.cdp_api_key_id:
        try:
            from fast_mcp_agent.blockchain import (
                BLOCKCHAIN_TOOLS,
                BlockchainService,
    ...
    from fast_mcp_agent.agents.orchestrator import OrchestratorAgent
    from fast_mcp_agent.agents.registry import AgentRegistry
    from fast_mcp_agent.agents.tools import DELEGATION_TOOLS, make_delegation_handler
    ...
    if (
        ...
        try:
            from fast_mcp_agent.slack.bolt_app import SlackBoltUI
...
async def chat(
    ...
    """
    from fast_mcp_agent.mcp.dependencies import get_orchestrator
